# CMSync mod — build & use (1.21.11 branch)

This branch (`main`) is QMSync `1.21.11` (a ChestTracker fork) with the CMSync
multiplayer overlay applied. The `26.1.2` branch is the same overlay on
ponuing/ChestTracker `26.1.2` — see [Branches](#branches) for the (small) differences.
...
Prereqs: **Java 21** + a GitHub classic token with `read:packages`
(JackFredLib/WhereIsIt come from GitHub Packages).
